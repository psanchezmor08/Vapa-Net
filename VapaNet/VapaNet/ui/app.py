...
class VapaNetApp:
    ...
    def initialize(self):
        ...
        p.window_min_height = 550
        p.fonts = {"Consolas": "Consolas"}
        p.theme = ft.Theme(color_scheme_seed=T.LIME, font_family="Segoe UI")
        ...
        topbar = ft.Container(
            content=ft.Row([
                ...
                ft.Row([
                    ft.Container(
                        content=ft.Text("V", size=16, weight=ft.FontWeight.W_500,
                                        color=T.DARK_BG),
                        bgcolor=T.LIME,
                        width=30, height=30,
                        border_radius=8,
                    ...
                    ft.Column([
                        ft.Text("VapaNet", size=15, weight=ft.FontWeight.W_500,
                                color=T.LIME),
        ...
        for key, label, icon, section in nav_def:
            ...
            container = ft.Container(
                ...
                content=ft.Row([
                    ft.Icon(icon, size=16,
                            color=T.LIME if key == "dashboard" else T.TEXT_SECONDARY),
                    ft.Text(label, size=13,
                            color=T.LIME if key == "dashboard" else T.TEXT_SECONDARY,
                            expand=True),
                ], spacing=9),
                bgcolor=T.DARK_HOVER if key == "dashboard" else "transparent",
                border=ft.border.all(1, T.DARK_BORDER2 if key == "dashboard" else "transparent"),
                border_radius=8,
            ...
            self._nav_items[key] = container
            sidebar_controls.append(ft.Container(content=container, padding=ft.padding.symmetric(horizontal=8)))
